...
public class EyeSet implements EyeFocus{
    ...
    //private Context mContext; //from extend frame
    ...
    ///for visualization
    public boolean VISUALIZATION_ON = true;
    public Path mBorderLinePath = new Path();
    public Path mFocusLines = new Path();
    ...
    private void initEyeSet() {
        ...
        ///for visualization
        if(VISUALIZATION_ON&&mEyeFrame!=null){
            mLinePaint.setStyle(Paint.Style.STROKE);
            mLinePaint.setColor(Color.BLACK);
            mLinePaint.setStrokeWidth(10);
            mEyeFrame.setWillNotDraw(false);
        }
    }


    public void addEyeball(int height,int width,float localx,float localy, EyeDraw eyeDraw){
        final EyeBall newEye = new EyeBall(mEyeFrame.getContext(),width,height,eyeDraw);
    ...
    private void creatBorderlines(EyeBall eye){
        ...
        for(int x=0;x<mNumEyeballs;x++){
            ...
            if(mEyeballs.get(x)!=eye){
                ...
                ///for visualization
                if(VISUALIZATION_ON){
                    mBorderLinePath.moveTo(borderLine.start.x, borderLine.start.y);
                    mBorderLinePath.lineTo(borderLine.end.x, borderLine.end.y);
    ...
    public void focusHere(int focusRequestX, int focusRequestY){
        ...
        if(POSITION_IS_FIXED && mNumEyeballs>1){
            ...
            //for visualization
            if(VISUALIZATION_ON)mFocusLines.addCircle(mEyeballs.get(primary).getmEyeballCenterWindowX(),mEyeballs.get(primary).getmEyeballCenterWindowY(),mEyeballs.get(primary).getHeight()*.6f, Path.Direction.CW);
            ...
            for(int x=0;x<mNumEyeballs;x++) if(x!=primary){
                ...
                //for visualization
                if(VISUALIZATION_ON){
                    mFocusLines.moveTo(focusRequestX,focusRequestY);
                    mFocusLines.lineTo(xOffset,yOffset);
                }
                ...
                if(intersect.intersects){ 
                    ...
                    //for visualization
                    if(VISUALIZATION_ON)mFocusLines.addCircle(intersect.intersectionPoint.x,intersect.intersectionPoint.y,15, Path.Direction.CW);
                }
